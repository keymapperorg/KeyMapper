...
class RoomGroupRepository
    ...
    constructor(
        ...
        override fun getKeyMapsByGroup(groupUid: String): Flow<KeyMapEntitiesWithGroup> =
            dao.getKeyMapsByGroup(groupUid).flowOn(dispatchers.io())

        override suspend fun getGroup(uid: String): GroupEntity? = withContext(dispatchers.io()) { dao.getById(uid) }

        override fun getAllGroups(): Flow<List<GroupEntity>> = dao.getAll().flowOn(dispatchers.io())

        override fun getGroups(vararg uid: String): Flow<List<GroupEntity>> = dao.getManyByIdFlow(*uid).flowOn(dispatchers.io())

        override fun getGroupsByParent(uid: String?): Flow<List<GroupEntity>> = dao.getGroupsByParent(uid).flowOn(dispatchers.io())

        override fun getGroupWithChildren(uid: String): Flow<GroupEntityWithChildren> =
            dao.getGroupWithSubGroups(uid).flowOn(dispatchers.io())
        ...
        override suspend fun setLastOpenedDate(
            groupUid: String,
            timestamp: Long,
        ) {
            withContext(dispatchers.io()) {
