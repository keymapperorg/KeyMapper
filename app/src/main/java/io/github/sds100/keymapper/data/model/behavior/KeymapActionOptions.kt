...
class KeymapActionOptions(action: Action,
    ...
    init {
        ...
        repeatRate = BehaviorOption(
            id = ActionBehavior.ID_REPEAT_RATE,
        ...
        repeatDelay = BehaviorOption(
            id = ActionBehavior.ID_REPEAT_DELAY,
        ...
        stopRepeatingWhenTriggerPressedAgain = BehaviorOption(
            id = ActionBehavior.ID_STOP_REPEATING_TRIGGER_PRESSED_AGAIN,
            value = stopRepeatingTriggerPressedAgain,
            isAllowed = repeat.value
        )

        stopRepeatingWhenTriggerReleased = BehaviorOption(
            id = ActionBehavior.ID_STOP_REPEATING_TRIGGER_RELEASED,
            value = !stopRepeatingTriggerPressedAgain,
            isAllowed = repeat.value
        )
        ...
        delayBeforeNextAction = BehaviorOption(
            id = ActionBehavior.ID_DELAY_BEFORE_NEXT_ACTION,
    ...
    override fun setValue(id: String, value: Int): BaseOptions<Action> {
        when (id) {
            ActionBehavior.ID_REPEAT_RATE -> repeatRate.value = value
            ActionBehavior.ID_REPEAT_DELAY -> repeatDelay.value = value
            ActionBehavior.ID_MULTIPLIER -> multiplier.value = value
            ActionBehavior.ID_DELAY_BEFORE_NEXT_ACTION -> delayBeforeNextAction.value = value
            ActionBehavior.ID_HOLD_DOWN_DURATION -> holdDownDuration.value = value
        }
        ...
        when (id) {
            ActionBehavior.ID_REPEAT -> {
                ...
                if (value && stopHoldDownWhenTriggerPressedAgain.value) {
                    setValue(ActionBehavior.ID_STOP_HOLD_DOWN_WHEN_TRIGGER_PRESSED_AGAIN, false)
                }

                holdDownDuration.isAllowed = holdDown.value && repeat.value
            }

            ActionBehavior.ID_SHOW_VOLUME_UI -> showVolumeUi.value = value
            ActionBehavior.ID_SHOW_PERFORMING_ACTION_TOAST -> showPerformingActionToast.value = value

            ActionBehavior.ID_STOP_REPEATING_TRIGGER_PRESSED_AGAIN -> {
            ...
            ActionBehavior.ID_STOP_REPEATING_TRIGGER_RELEASED -> {
            ...
            ActionBehavior.ID_STOP_HOLD_DOWN_WHEN_TRIGGER_RELEASED -> {
                stopHoldDownWhenTriggerReleased.value = value
                stopHoldDownWhenTriggerPressedAgain.value = !value
            }

            ActionBehavior.ID_STOP_HOLD_DOWN_WHEN_TRIGGER_PRESSED_AGAIN -> {
                stopHoldDownWhenTriggerPressedAgain.value = value
                stopHoldDownWhenTriggerReleased.value = !value
            }
        }
