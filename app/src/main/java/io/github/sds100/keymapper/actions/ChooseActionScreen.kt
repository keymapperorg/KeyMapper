...
fun ChooseActionScreen(
    ...
    onActionChosen: (ActionData) -> Unit,
    navigateBack: () -> Unit
) {
    Scaffold(modifier, bottomBar = {
        ChooseActionAppBar(navigateBack, viewModel.query, onSearchQueryChanged = { viewModel.query = it })
    }) { padding ->
        ChooseActionList(
            Modifier
                .padding(padding)
...
private fun ChooseActionScreenPreview() {
    MaterialTheme {
        ChooseActionScreen(onActionChosen = {}, navigateBack = {})
    }
}

@Composable
private fun ChooseActionList(modifier: Modifier = Modifier, listItems: List<ChooseActionListItem>) {
    ...
    LazyColumn(
        ...
        items(listItems) {
            SimpleListItem(modifier = Modifier.fillMaxWidth(), icon = it.icon, title = it.title)
        }
    }
}

@Composable
private fun ChooseActionAppBar(navigateBack: () -> Unit, searchQuery: String, onSearchQueryChanged: (String) -> Unit) {
