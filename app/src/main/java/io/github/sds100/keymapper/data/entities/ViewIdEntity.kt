...
data class ViewIdEntity(
    ...
    @SerializedName(NAME_VIEW_ID)
    val viewId: String,
    @ColumnInfo(name = ViewIdDao.KEY_PACKAGE_NAME)
    @SerializedName(NAME_PACKAGE_NAME)
    val packageName: String,
    @ColumnInfo(name = ViewIdDao.KEY_FULL_NAME)
    @SerializedName(NAME_FULL_NAME)
    val fullName: String,
){
    companion object{
        const val NAME_ID = "id"
        const val NAME_VIEW_ID = "view_id"
        const val NAME_PACKAGE_NAME = "package_name"
        const val NAME_FULL_NAME = "full_name"
    }
}