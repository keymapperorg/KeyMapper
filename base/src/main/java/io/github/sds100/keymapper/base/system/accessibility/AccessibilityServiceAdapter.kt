...
@Singleton
class AccessibilityServiceAdapter<E>(
    ...
    override val eventReceiver = MutableSharedFlow<E>()

    val eventsToService = MutableSharedFlow<E>()
    ...
    override fun sendAsync(event: E) {
        coroutineScope.launch {
            eventsToService.emit(event)
        }
    }

    override suspend fun send(event: E): Result<*> {
        state.value = getState()
