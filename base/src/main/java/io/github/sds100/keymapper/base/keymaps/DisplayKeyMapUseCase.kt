...
class DisplayKeyMapUseCaseImpl
    ...
    constructor(
         ...
         */
        private val purchasesFlow: Flow<State<KMResult<Set<ProductId>>>> =
            callbackFlow {
                try {
                    val value =
                        withTimeout(5000L) {
                            purchasingManager.purchases
                                .filterIsInstance<State.Data<KMResult<Set<ProductId>>>>()
                                .first()
                        }
         ...
         * the errors.
         */
        override val triggerErrorSnapshot: Flow<TriggerErrorSnapshot> =
            combine(
                merge(
        ...
        override suspend fun isFloatingButtonsPurchased(): Boolean =
            purchasingManager.isPurchased(ProductId.FLOATING_BUTTONS).valueIfFailure {
                false
            }
        ...
        override fun getInputMethodLabel(imeId: String): KMResult<String> = inputMethodAdapter.getInfoById(imeId).then { Success(it.label) }
        ...
        override fun getRingtoneLabel(uri: String): KMResult<String> = ringtoneAdapter.getLabel(uri)
    }
