...
class ActionUiHelper(
    ...
    fun getTitle(
        ...
        when (action) {
            ...
            is ActionData.SwitchKeyboard ->
                getInputMethodLabel(action.imeId).handle(
            ...
            is ActionData.PinchScreen ->
                if (action.description.isNullOrBlank()) {
                    val pinchTypeDisplayName =
                        if (action.pinchType == PinchScreenType.PINCH_IN) {
            ...
            is ActionData.Rotation.CycleRotations -> {
                val orientationStrings =
                    action.orientations.map { orientation ->
                        when (orientation) {
    ...
    fun getDrawableIcon(action: ActionData): IconInfo? =
        when (action) {
            ...
            is ActionData.Sound ->
                ...
                ActionUtils.getDrawableIcon(action.id)?.let { iconRes ->
                    IconInfo(
                        getDrawable(iconRes),
                        TintType.OnSurface,
                    )
