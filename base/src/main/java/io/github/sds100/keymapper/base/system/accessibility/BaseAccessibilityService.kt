...
abstract class BaseAccessibilityService :
    ...
    override fun performTalkBackGesture(gesture: TalkBackGestureType): KMResult<*> {
        ...
        when (gesture) {
            TalkBackGestureType.SWIPE_UP ->
                gestureBuilder.addStroke(buildSwipe(cx, cy, cx, cy - swipeLen, 200))

            TalkBackGestureType.SWIPE_DOWN ->
                gestureBuilder.addStroke(buildSwipe(cx, cy, cx, cy + swipeLen, 200))

            TalkBackGestureType.SWIPE_LEFT ->
                gestureBuilder.addStroke(buildSwipe(cx, cy, cx - swipeLen, cy, 200))

            TalkBackGestureType.SWIPE_RIGHT ->
                gestureBuilder.addStroke(buildSwipe(cx, cy, cx + swipeLen, cy, 200))

            TalkBackGestureType.SWIPE_UP_THEN_DOWN -> {
                val s1 = buildChainedSwipe(cx, cy, cx, cy - swipeLen, 150, willContinue = true)
                val s2 = s1.continueStroke(buildPath(cx, cy - swipeLen, cx, cy), 150, 150, false)
            ...
            TalkBackGestureType.SWIPE_LEFT_THEN_RIGHT -> {
                val s1 = buildChainedSwipe(cx, cy, cx - swipeLen, cy, 150, willContinue = true)
                val s2 = s1.continueStroke(buildPath(cx - swipeLen, cy, cx, cy), 150, 150, false)
                gestureBuilder.addStroke(s1).addStroke(s2)
            }
            ...
            TalkBackGestureType.SWIPE_RIGHT_THEN_UP -> {
                ...
                val s2 = s1.continueStroke(
                    buildPath(cx + swipeLen, cy, cx, cy - swipeLen),
                    150,
                    150,
                    false,
                )
            ...
            TalkBackGestureType.TWO_FINGER_DOUBLE_TAP_HOLD ->
                addMultiFingerDoubleTapHold(gestureBuilder, cx, cy, fingerSpacing, fingerCount = 2)
            ...
            TalkBackGestureType.TWO_FINGER_TRIPLE_TAP_HOLD ->
                addMultiFingerTripleTapHold(gestureBuilder, cx, cy, fingerSpacing, fingerCount = 2)
            ...
            TalkBackGestureType.THREE_FINGER_TRIPLE_TAP_HOLD ->
                addMultiFingerTripleTapHold(gestureBuilder, cx, cy, fingerSpacing, fingerCount = 3)
        ...
        val success = dispatchGesture(gestureBuilder.build(), null, gestureHandler)
        return if (success) Success(Unit) else KMError.FailedToDispatchGesture
    }

    private fun buildPath(x1: Float, y1: Float, x2: Float, y2: Float): Path = Path().apply {
        moveTo(x1, y1)
        lineTo(x2, y2)
    }
    ...
    private fun buildSwipe(
        ...
        y2: Float,
        duration: Long,
    ): StrokeDescription = StrokeDescription(buildPath(x1, y1, x2, y2), 0, duration)

    private fun buildChainedSwipe(
        x1: Float,
        y1: Float,
        x2: Float,
        y2: Float,
    ...
    private fun addMultiFingerTaps(
    ...
    private fun addMultiFingerDoubleTapHold(
        builder: GestureDescription.Builder,
        cx: Float,
        cy: Float,
        spacing: Float,
    ...
    private fun addMultiFingerTripleTapHold(
        builder: GestureDescription.Builder,
        cx: Float,
        cy: Float,
        spacing: Float,
    ...
    private fun addMultiFingerSwipe(
        builder: GestureDescription.Builder,
        xStart: Float,
        yStart: Float,
        xEnd: Float,
