...
class BackupManagerImpl
    ...
    constructor(
        ...
        private val backupAutomatically: Flow<Boolean> =
        ...
        override suspend fun backupKeyMaps(
            output: IFile,
            keyMapIds: List<String>,
        ): KMResult<Unit> =
        ...
        override suspend fun backupEverything(output: IFile): KMResult<Unit> =
            withContext(dispatchers.io()) {
                ...
                val layouts =
                    floatingLayoutRepository.layouts
        ...
        private suspend fun parseBackupContent(jsonFile: InputStream): KMResult<BackupContent> =
            withContext(dispatchers.io()) {
                try {
                    ...
                    // do nothing because this added the log table
                    val newFingerprintMapMigrations =
                        listOf(
                            JsonMigration(12, 13) { json -> json },
                        )
                    ...
                    if (rootElement.contains(BackupContent.NAME_FINGERPRINT_MAP_LIST) && backupDbVersion >= 12) {
                        rootElement.get(BackupContent.NAME_FINGERPRINT_MAP_LIST).asJsonArray.forEach { fingerprintMap ->
                            val migratedFingerprintMapJson =
                                MigrationUtils.migrate(
        ...
        suspend fun restore(
            ...
            try {
                ...
                if (backupContent.groups != null) {
                    ...
                    val groupUids =
                        backupContent.groups
                            .map { it.uid }
                            .toMutableSet()
                            .plus(existingGroupUids)

                    // Set the parents to null of any groups that have a missing parent.
                    val backupGroups =
                        backupContent.groups.map { group ->
        ...
        private suspend fun backupAsync(
            ...
            return withContext(dispatchers.io()) {
                ...
                try {
                    ...
                    return@withContext fileAdapter
                        .createZipFile(output, filesToBackup)
                        .then { Success(output) }
                } catch (e: Exception) {
