    ...
    return when (this) {
        is SystemError.PermissionDenied -> {
            val resId = when (permission) {
                ...
                Permission.CAMERA ->
                    R.string.error_action_requires_camera_permission

                Permission.DEVICE_ADMIN ->
                    R.string.error_need_to_enable_device_admin

                Permission.READ_PHONE_STATE ->
                    R.string.error_action_requires_read_phone_state_permission

                Permission.ACCESS_NOTIFICATION_POLICY ->
                    R.string.error_action_notification_policy_permission

                Permission.WRITE_SECURE_SETTINGS ->
                    R.string.error_need_write_secure_settings_permission

                Permission.NOTIFICATION_LISTENER ->
                    R.string.error_denied_notification_listener_service_permission

                Permission.CALL_PHONE ->
                    R.string.error_denied_call_phone_permission

                Permission.SEND_SMS ->
                    R.string.error_denied_send_sms_permission

                Permission.ROOT ->
                    R.string.error_requires_root

                Permission.IGNORE_BATTERY_OPTIMISATION ->
                    R.string.error_battery_optimisation_enabled

                Permission.SHIZUKU ->
                    R.string.error_shizuku_permission_denied

                Permission.ACCESS_FINE_LOCATION ->
                    R.string.error_access_fine_location_permission_denied

                Permission.ANSWER_PHONE_CALL ->
                    R.string.error_answer_end_phone_call_permission_denied
        ...
        is KMError.NoCompatibleImeChosen ->
            resourceProvider.getString(
                R.string.error_ime_must_be_chosen,
            )

        is KMError.SystemFeatureNotSupported ->
            when (this.feature) {
        ...
        is DataError.ExtraNotFound ->
            resourceProvider.getString(
                R.string.error_extra_not_found,
                extraId,
            )

        is KMError.SdkVersionTooLow ->
            resourceProvider.getString(
                R.string.error_sdk_version_too_low,
                BuildUtils.getSdkVersionName(minSdk),
            )

        is KMError.SdkVersionTooHigh ->
            resourceProvider.getString(
                R.string.error_sdk_version_too_high,
                BuildUtils.getSdkVersionName(maxSdk),
            )

        is KMError.InputMethodNotFound ->
            resourceProvider.getString(
                R.string.error_ime_not_found,
                imeLabel,
            )

        is KMError.FrontFlashNotFound ->
            resourceProvider.getString(
                R.string.error_front_flash_not_found,
            )

        is KMError.BackFlashNotFound ->
            resourceProvider.getString(
                R.string.error_back_flash_not_found,
            )

        is KMError.DeviceNotFound ->
            resourceProvider.getString(R.string.error_device_not_found)

        is KMError.Exception ->
            exception.toString()

        is KMError.EmptyJson ->
            resourceProvider.getString(R.string.error_empty_json)

        is KMError.InvalidNumber ->
            resourceProvider.getString(R.string.error_invalid_number)

        is KMError.NumberTooSmall ->
            resourceProvider.getString(
        ...
        KMError.BackupVersionTooNew ->
            resourceProvider.getString(
                R.string.error_backup_version_too_new,
            )

        KMError.NoIncompatibleKeyboardsInstalled ->
            resourceProvider.getString(
                R.string.error_no_incompatible_input_methods_installed,
            )

        KMError.NoMediaSessions ->
            resourceProvider.getString(R.string.error_no_media_sessions)

        KMError.NoVoiceAssistant ->
            resourceProvider.getString(
        ...
        AccessibilityServiceError.Crashed ->
            resourceProvider.getString(
                R.string.error_accessibility_service_crashed,
            )

        KMError.LauncherShortcutsNotSupported ->
            resourceProvider.getString(
        ...
        KMError.FailedToFindAccessibilityNode ->
            resourceProvider.getString(
                R.string.error_failed_to_find_accessibility_node,
            )

        is KMError.FailedToPerformAccessibilityGlobalAction ->
            resourceProvider.getString(
        ...
        KMError.AppShortcutCantBeOpened ->
            resourceProvider.getString(
                R.string.error_opening_app_shortcut,
            )

        KMError.InsufficientPermissionsToOpenAppShortcut ->
            resourceProvider.getString(
                R.string.error_keymapper_doesnt_have_permission_app_shortcut,
            )

        KMError.NoAppToPhoneCall ->
            resourceProvider.getString(R.string.error_no_app_to_phone_call)

        KMError.NoAppToSendSms ->
            resourceProvider.getString(R.string.error_no_app_to_send_sms)
        ...
        is KMError.SendSmsError -> {
            when (resultCode) {
                ...
                SmsManager.RESULT_RIL_SIM_ABSENT ->
                    ...
                    resourceProvider.getString(R.string.error_sms_generic_failure)
            }
        }

        KMError.CameraInUse ->
            resourceProvider.getString(R.string.error_camera_in_use)

        KMError.CameraError ->
            resourceProvider.getString(R.string.error_camera_error)

        KMError.CameraDisabled ->
            resourceProvider.getString(R.string.error_camera_disabled)

        KMError.CameraDisconnected ->
            resourceProvider.getString(R.string.error_camera_disconnected)

        KMError.MaxCamerasInUse ->
            resourceProvider.getString(R.string.error_max_cameras_in_use)

        KMError.CameraVariableFlashlightStrengthUnsupported ->
            resourceProvider.getString(
        ...
        KMError.SwitchImeFailed ->
            resourceProvider.getString(R.string.error_failed_to_change_ime)
        ...
        KMError.NoDeviceAssistant ->
            resourceProvider.getString(R.string.error_no_device_assistant)

        KMError.NoSettingsApp ->
            resourceProvider.getString(R.string.error_no_settings_app)

        KMError.NoAppToOpenUrl ->
            resourceProvider.getString(R.string.error_no_app_to_open_url)

        KMError.CantFindSoundFile ->
            resourceProvider.getString(R.string.error_cant_find_sound_file)

        is KMError.CorruptJsonFile ->
            reason

        is KMError.CannotCreateFileInTarget ->
            resourceProvider.getString(
                R.string.error_file_access_denied,
                uri,
            )

        KMError.FileOperationCancelled ->
            resourceProvider.getString(
                R.string.error_file_operation_cancelled,
            )

        is KMError.NoSpaceLeftOnTarget ->
            resourceProvider.getString(
                R.string.error_no_space_left_at_target,
                uri,
            )

        is KMError.NotADirectory ->
            resourceProvider.getString(R.string.error_not_a_directory, uri)

        is KMError.NotAFile ->
            resourceProvider.getString(R.string.error_not_a_file, uri)

        is KMError.SourceFileNotFound ->
            resourceProvider.getString(
                R.string.error_source_file_not_found,
                uri,
            )

        KMError.StoragePermissionDenied ->
            resourceProvider.getString(
        ...
        is KMError.TargetDirectoryNotFound ->
            resourceProvider.getString(
                R.string.error_directory_not_found,
                uri,
            )

        is KMError.TargetFileNotFound ->
            resourceProvider.getString(
        ...
        KMError.NoFileName ->
            resourceProvider.getString(R.string.error_no_file_name)
        ...
        KMError.GestureStrokeCountTooHigh ->
            resourceProvider.getString(
                R.string.trigger_error_gesture_stroke_count_too_high,
            )

        KMError.GestureDurationTooHigh ->
            resourceProvider.getString(
                R.string.trigger_error_gesture_duration_too_high,
            )

        KMError.DpadTriggerImeNotSelected ->
            resourceProvider.getString(
                R.string.trigger_error_dpad_ime_not_selected,
            )

        KMError.InvalidBackup ->
            resourceProvider.getString(R.string.error_invalid_backup)

        KMError.MalformedUrl ->
            resourceProvider.getString(R.string.error_malformed_url)

        KMError.UiElementNotFound ->
            resourceProvider.getString(R.string.error_ui_element_not_found)

        is KMError.ShellCommandTimeout ->
            resourceProvider.getString(
        ...
        PurchasingError.PurchasingProcessError.Cancelled ->
            resourceProvider.getString(
                R.string.purchasing_error_cancelled,
            )

        PurchasingError.PurchasingProcessError.NetworkError ->
            resourceProvider.getString(
                R.string.purchasing_error_network,
            )

        PurchasingError.PurchasingProcessError.ProductNotFound ->
            resourceProvider.getString(
                R.string.purchasing_error_product_not_found,
            )

        PurchasingError.PurchasingProcessError.StoreProblem ->
            resourceProvider.getString(
                R.string.purchasing_error_store_problem,
            )

        PurchasingError.PurchasingProcessError.PaymentPending ->
            resourceProvider.getString(
                R.string.purchasing_error_payment_pending,
            )

        PurchasingError.PurchasingProcessError.PurchaseInvalid ->
            resourceProvider.getString(
        ...
        is KMError.KeyMapperSmsRateLimit ->
            resourceProvider.getString(
        ...
        is KMError.MediaActionUnsupported ->
            resourceProvider.getString(R.string.error_media_action_unsupported)
