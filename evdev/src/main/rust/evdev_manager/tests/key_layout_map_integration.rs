...
fn test_load_generic_kl() {
    let file_path = format!("{}/Generic.kl", get_test_data_dir());
    let map = KeyLayoutMap::load_from_file(&file_path).expect("Failed to load Generic.kl");
    
    // Test some basic key mappings
    let (key_code, _flags) = map.map_key(1).expect("Scan code 1 should map to ESCAPE");
    assert_eq!(key_code, 111); // ESCAPE
    
    let (key_code, _flags) = map.map_key(2).expect("Scan code 2 should map to 1");
    assert_eq!(key_code, 8); // KEYCODE_1
    
    let (key_code, _flags) = map.map_key(15).expect("Scan code 15 should map to TAB");
...
fn test_parse_all_key_entries() {
    ...
    for (scan_code, expected_key_code) in test_cases {
        let (key_code, _flags) = map.map_key(scan_code)
            .unwrap_or_else(|_| panic!("Scan code {} should be mapped", scan_code));
        assert_eq!(key_code, expected_key_code, "Scan code {} should map to key code {}", scan_code, expected_key_code);
    }
}
...
fn test_parse_axis_entries() {
    ...
    assert_eq!(axis_info.mode, evdev_manager::key_layout_map::KeyLayoutAxisMode::Normal);
...
fn test_map_axis_functionality() {
    ...
    assert_eq!(axis_info.high_axis, -1);
    assert_eq!(axis_info.split_value, 0);
    assert_eq!(axis_info.flat_override, -1);
...
fn test_find_scan_codes_for_key() {
    ...
    assert!(scan_codes.contains(&1), "Should find scan code 1 for ESCAPE");
    assert!(!scan_codes.contains(&465), "Should not find scan code 465 (has FUNCTION flag)");
    
    // Find scan codes for a common key
    let scan_codes = map.find_scan_codes_for_key(8); // KEYCODE_1
    assert!(scan_codes.contains(&2), "Should find scan code 2 for KEYCODE_1");
}

#[test]
fn test_load_all_files_in_test_data() {
    let test_data_dir = Path::new(get_test_data_dir());
    if !test_data_dir.exists() {
        panic!("Test data directory does not exist: {}", get_test_data_dir());
    }
