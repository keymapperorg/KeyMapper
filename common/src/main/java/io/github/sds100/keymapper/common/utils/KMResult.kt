...
abstract class KMError : KMResult<Nothing>() {
    ...
    data class SystemFeatureNotSupported(
        val feature: String,
    ) : KMError()

    data class SdkVersionTooLow(
        val minSdk: Int,
    ) : KMError()

    data class SdkVersionTooHigh(
        val maxSdk: Int,
    ) : KMError()

    data class InputMethodNotFound(
        val imeLabel: String,
    ) : KMError()
    ...
    data class NumberTooBig(
        val max: Int,
    ) : KMError()

    data class NumberTooSmall(
        val min: Int,
    ) : KMError()
    ...
    data class FailedToPerformAccessibilityGlobalAction(
        val action: Int,
    ) : KMError()
    ...
    data class CannotCreateFileInTarget(
        val uri: String,
    ) : KMError()

    data class SourceFileNotFound(
        val uri: String,
    ) : KMError()

    data class TargetFileNotFound(
        val uri: String,
    ) : KMError()

    data class TargetDirectoryNotFound(
        val uri: String,
    ) : KMError()
    ...
    data class NoSpaceLeftOnTarget(
        val uri: String,
    ) : KMError()
    ...
    data class KeyEventActionError(
        val baseError: KMError,
    ) : KMError()

    data class ShellCommandTimeout(
...
inline infix fun <T, U> KMResult<T>.then(f: (T) -> KMResult<U>) =
    when (this) {
        is Success -> f(this.value)
        is KMError -> this
    }

suspend infix fun <T, U> KMResult<T>.suspendThen(f: suspend (T) -> KMResult<U>) =
    when (this) {
        is Success -> f(this.value)
        is KMError -> this
    }

inline infix fun <T> KMResult<T>.otherwise(f: (error: KMError) -> KMResult<T>) =
    when (this) {
        is Success -> this
        is KMError -> f(this)
    }
...
inline infix fun <T> KMResult<T>.valueIfFailure(f: (error: KMError) -> T): T =
    when (this) {
